...
public class ChunkData
{
    ...
    {
        Vector3Int offset = Direction.AxisToVec(direction);
        Vector3Int pos = new(x, y, z);
        Vector3Int oPos = pos + offset + position * Chunk.Size;

        if (oPos.x is < 0 or >= World.WidthInBlocks ||
            oPos.y is < 0 or >= World.HeightInBlocks ||
            oPos.z is < 0 or >= World.WidthInBlocks) return;
        
        if (world.GetBlock(oPos) == Block.Id.Air) GenFace(blockId, x, y, z, direction);
    }
    ...
    private void GenFace(Block.Id blockId, int x, int y, int z, Direction.Axis direction)
    {
        ...
        for (int i = 0; i < faceVerticesLength; i++)
        {
            ...
            Direction.Axis uvDirection = direction;

            if (blockId == Block.Id.Die)
            {
                uvDirection = (Direction.Axis)(((int)blockId + x + y + z) % 6);
            }
            
            Vector3 offsetUv = new(originalUv.x, originalUv.y, Block.GetTextureId(blockId, uvDirection));
